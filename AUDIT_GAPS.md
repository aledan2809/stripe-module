...
| G-STRIPE-010 | P1 | `src/server/sync.ts` (`fetchProjectProducts`) | `syncPlans` își bazează idempotența pe `stripe.products.search`, care e **eventually-consistent (~16-60s) și flapping între replici** — verificat empiric 2026-06-11: chiar și DUPĂ ce un read a văzut produsul, sync-ul imediat următor a creat un duplicat (6 duplicate într-un singur test). Consumatorii care apelează syncPlans la deploy-uri consecutive pot duplica produsele din contul Stripe. Fix: înlocuiește search cu `stripe.products.list` (strongly-consistent) + filtrare client-side pe metadata, sau idempotency key local. **Fix pe lib → propose-confirm-apply per §6.1 (BlocHub NO-TOUCH consumer).** | OPEN |
